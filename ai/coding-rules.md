...
> **教训来源：2026-07-17 事故。** `repackDmgForEntitlements` 任务在 staging 目录创建了 `Applications -> /Applications` 符号链接，随后用 `staging.deleteRecursively()` 清理。`deleteRecursively` 递归遍历时若跟随符号链接，会进入真实的 `/Applications` 递归删除，导致用户已安装的第三方 app 全部丢失（IntelliJ/Docker/VSCode/PyCharm/WebStorm 等，仅配置数据幸存）。此类符号链接 + 递归删除的组合是致命隐患，必须从规则上根除。
...
任何对 `shared/build.gradle.kts`、`build.gradle.kts`、`scripts/`、自定义 Gradle 任务的修改，提交前必须逐条对照：
