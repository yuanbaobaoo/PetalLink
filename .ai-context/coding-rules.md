...
函数和方法内部不能只依赖声明上方的 JSDoc：

- 函数体包含多个业务阶段，或涉及校验、状态变更、网络/数据库/文件 I/O、锁、重试、回滚、事件发布时，必须在对应代码前使用简短的 `//` 行注释标明阶段目的或关键约束。
- 连续业务代码超过约 15 行时，应按“准入校验、读取、执行、持久化、通知、失败恢复”等真实职责分段；没有实际阶段边界时不强行凑数。
- 行注释应说明“为什么这样处理、此处保证什么、失败后怎样”，禁止仅把下一行代码翻译成中文。
- 简单赋值、显而易见的条件和纯展示拼装不机械添加注释，避免用注释数量掩盖控制流问题。
...
- 函数或方法包含多个业务阶段，或涉及校验、状态变更、HTTP/数据库/文件 I/O、锁、`await`、重试、回滚、事件发布时，必须在对应代码前使用简短的 `//` 行注释标明阶段目的或关键约束，不能只在声明上方写 `///`。
- 连续业务代码超过约 15 行时，应按“准入校验、读取、执行、持久化、通知、失败恢复”等真实职责分段；没有实际阶段边界时不强行凑数。
