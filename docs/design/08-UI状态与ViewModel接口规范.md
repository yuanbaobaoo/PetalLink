...
> 原项目前端为 Vue3 + Pinia。CMP 重构后，这些「API 调用」变成 **ViewModel → 业务层的方法**，「Pinia store」变成 **ViewModel/StateFlow**。本文档记录前端完整类型接口规范与状态管理逻辑，供重构对照。
>
...
3. **模块级状态 + Promise resolver**（useDialog/useToast）：命令式 `await confirmDialog()` 是核心交互模式，Flutter 需等价机制（如 Completer/回调）
4. **sidebarRefresh 计数器**：因 Vue watch 对布尔无法重复触发，用递增数字触发刷新；Flutter 若用 Rx/Stream 需注意等价问题
