...
> **来源**：petal-link-cmp `docs/plan/07`（2026-07 纳入 Flutter 项目）。文件身份已按 inode 方案修订（schemaVersion=6，xattr 键 5→2）；文中 Kotlin 代码为等价示意，Flutter 实现为 Dart + MethodChannel。
...
> 本章已按 **inode 方案**（`schemaVersion=6`）修订。文件身份识别从「fileId xattr」改为「inode + DB 映射」，xattr 键从 5 个精简到 2 个。完整方案见 `10-基于inode的文件身份识别方案.md`。
...
| `mark_downloaded` | `state` → `downloaded` + 清除 Finder 灰标。**下载覆盖产生新 inode → 调用 `upsertMapping` 更新映射**（确定性记账，见 `10` §五） |
...
> 好处：暂存文件与恢复记录在同一 DB 事务内，消除「文件已暂存但 xattr 未写」的窗口（见 `10` §4.8）。
...
- inode 在下载覆盖 / 跨卷移动 / 断电恢复等场景可能漂移；方案对「下载覆盖」做确定性兜底（`upsertMapping`），对「断电/还原漂移」用 size + mtime 二级校验保守确认（见 `10` §6）。
