...
## 七、关键重构提示（给 Compose Multiplatform (CMP)）
...
3. **模块级状态 + Promise resolver**（useDialog/useToast）：命令式 `await confirmDialog()` 是核心交互模式，Compose Multiplatform (CMP) 需等价机制（如回调/Channel）
4. **sidebarRefresh 计数器**：因 Vue watch 对布尔无法重复触发，用递增数字触发刷新；Compose Multiplatform (CMP) 若用 Flow/State 需注意等价问题
