...
> **来源**：petal-link（Tauri/Rust 原版）`docs/`，原文保留，作为业务逻辑基线参考。
> **注意**：本文描述的 xattr fileId 文件身份方案已被 **inode 方案**取代（见 `docs/design/10-基于inode的文件身份识别方案.md`），涉及文件身份的章节请以 design 系列为准。
