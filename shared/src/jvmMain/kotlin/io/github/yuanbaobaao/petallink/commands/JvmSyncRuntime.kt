...
import io.github.yuanbaobaao.petallink.config.JvmMountPaths
...
import io.github.yuanbaobaao.petallink.mount.JvmLocalFileScanner
import io.github.yuanbaobaao.petallink.mount.JvmLocalWatcher
...
import io.github.yuanbaobaao.petallink.sync.Planner
...
import io.github.yuanbaobaao.petallink.sync.ConflictResolver
import io.github.yuanbaobaao.petallink.sync.engine.ActionPlannerGuards
import io.github.yuanbaobaao.petallink.sync.engine.ActivityTracker
import io.github.yuanbaobaao.petallink.sync.engine.BfsCloudTreeRefresher
import io.github.yuanbaobaao.petallink.sync.engine.CloudTreeCache
import io.github.yuanbaobaao.petallink.sync.engine.CycleCoordinator
import io.github.yuanbaobaao.petallink.sync.engine.CycleRequest
import io.github.yuanbaobaao.petallink.sync.engine.CycleRequestDispatcher
import io.github.yuanbaobaao.petallink.sync.engine.JvmCloudTreeCheckpointStore
import io.github.yuanbaobaao.petallink.sync.engine.JvmFreeUpService
import io.github.yuanbaobaao.petallink.sync.engine.FilesApiFreeUpVerifier
import io.github.yuanbaobaao.petallink.sync.engine.RuntimeStatus
import io.github.yuanbaobaao.petallink.sync.engine.StatusAggregator
...
import io.github.yuanbaobaao.petallink.sync.engine.TransferOperationsImpl
import io.github.yuanbaobaao.petallink.sync.engine.UploadFailedEvent
import io.github.yuanbaobaao.petallink.sync.engine.JvmTransferFileStore
import io.github.yuanbaobaao.petallink.sync.engine.TaskRunner
import io.github.yuanbaobaao.petallink.sync.engine.TaskContext
import io.github.yuanbaobaao.petallink.sync.engine.TaskDisposition
import io.github.yuanbaobaao.petallink.sync.engine.JvmRemoteTransferVerifier
...
class JvmSyncRuntime(
    ...
    private suspend fun runCycleOwned(request: CycleRequest): Result<Unit> {
        ...
        if (request.contains(CycleRequest.RETRY)) {
            ...
            if (ids.isEmpty()) {
                val retryable = db.transfers.selectByState(io.github.yuanbaobaao.petallink.sync.TransferState.Failed) +
                    db.transfers.selectByState(io.github.yuanbaobaao.petallink.sync.TransferState.RestartRequired)
    ...
    private suspend fun executeAction(
        ...
        return try {
            ...
            SyncActionType.MOVE_IN_CLOUD -> {
                ...
                val currentParent = runCatching { io.github.yuanbaobaao.petallink.drive.DriveParsers.singleParent(remote) }.getOrNull()
    ...
    private suspend fun executeTransferAction(
        ...
        val id = active?.id ?: db.transfers.insert(
            TransferTask(
                ...
                totalSize = total,
                state = io.github.yuanbaobaao.petallink.sync.TransferState.Pending,
                errorMessage = null,
        ...
        val disposition = when (current.state) {
            io.github.yuanbaobaao.petallink.sync.TransferState.Pending,
            io.github.yuanbaobaao.petallink.sync.TransferState.WaitingForNetwork,
            io.github.yuanbaobaao.petallink.sync.TransferState.BackingOff -> taskRunner.runExpected(taskContext(current))
            io.github.yuanbaobaao.petallink.sync.TransferState.Completed -> TaskDisposition.COMPLETED
            io.github.yuanbaobaao.petallink.sync.TransferState.Failed -> TaskDisposition.FAILED
            io.github.yuanbaobaao.petallink.sync.TransferState.RestartRequired -> TaskDisposition.RESTART_REQUIRED
            io.github.yuanbaobaao.petallink.sync.TransferState.VerifyingRemote -> TaskDisposition.VERIFYING_REMOTE
            io.github.yuanbaobaao.petallink.sync.TransferState.Running -> TaskDisposition.BLOCKED
            io.github.yuanbaobaao.petallink.sync.TransferState.Canceled -> TaskDisposition.CANCELED
        }
    ...
    private suspend fun syncFolderSubtree(folderId: String, relativePath: String) {
        ...
        while (queue.isNotEmpty()) {
            ...
            for (file in filesApi.listAllFiles(parentId)) {
                ...
                if (io.github.yuanbaobaao.petallink.drive.DriveParsers.isFolderMime(file.mimeType)) {
    ...
    private suspend fun resumePendingTransfers(taskRunner: TaskRunner) {
        for (task in db.transfers.selectByState(io.github.yuanbaobaao.petallink.sync.TransferState.Pending)) {
    ...
    private suspend fun activeTransfer(relativePath: String, direction: TransferDirection): TransferTask? {
        val states = listOf(
            io.github.yuanbaobaao.petallink.sync.TransferState.Pending,
            io.github.yuanbaobaao.petallink.sync.TransferState.Running,
            io.github.yuanbaobaao.petallink.sync.TransferState.WaitingForNetwork,
            io.github.yuanbaobaao.petallink.sync.TransferState.BackingOff,
            io.github.yuanbaobaao.petallink.sync.TransferState.VerifyingRemote,
            io.github.yuanbaobaao.petallink.sync.TransferState.RestartRequired,
        )
